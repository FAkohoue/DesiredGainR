# Empirical validation with real breeding-programme data
...
foundation, multi-environment transport and recommendation workflow. A
future prospective comparison assigning comparable populations to
alternative vectors would add direct estimates of incremental field
response between strategies; it is a next level of evidence, not a
prerequisite for using the package’s current model-based decision
support.

## Headline results

| dataset | endpoint | estimate | lower | upper | unit | status |
|:---|:---|---:|---:|---:|:---|:---|
| CNA6 rice recurrent selection | yld candidate-minus-check trend | 234.784 | 32.263 | 437.306 | yld units per cycle | supportive |
| Maize haploid-inducer recurrent selection | HIF connected cycle trend | 0.565 | 0.134 | 0.996 | HIF percentage points per cycle | supportive |
| CIMMYT rapid-cycle wheat genomic selection | P+RKHS-KA predicted yield trend | 0.158 | 0.013 | 0.303 | t/ha per cycle | descriptive |
| CIMMYT rapid-cycle wheat genomic selection | published observed yield trend | 0.265 | -0.015 | 0.545 | t/ha per cycle | external-anchor |
| INIA historical rice breeding programme | E1 index score versus advancement within four years | 0.773 | 0.731 | 0.819 | rank AUC | programme-concordant |
| INIA historical rice breeding programme | multi-trait AUC minus yield-only AUC | 0.017 | 0.004 | 0.031 | AUC difference | incremental-support |
| INIA historical rice breeding programme | top-20-percent advancement enrichment | 1.774 | 1.572 | 2.039 | rate ratio | programme-concordant |
| Genomes-to-Fields maize | early versus late frozen-index score | 0.556 | 0.329 | 0.722 | Spearman correlation | supportive |
| CIMMYT maize RCGS (Zhang et al. 2017) | grain-yield gain slope C0-C4 | 0.175 | NA | NA | t/ha per cycle | programme-concordant |
| CIMMYT maize RCGS (Zhang et al. 2017) | C0 grain-yield genomic prediction from genome-wide marker sample | 0.484 | 0.469 | 0.495 | repeated five-fold predictive correlation | programme-concordant |
| CIMMYT maize RCGS (Zhang et al. 2017) | optimised versus equal-direction worst-trait response | 0.005 | -0.041 | 0.085 | favourable within-environment SD | mixed |

The evidence is informative because it distinguishes strong findings
from design-dependent ones. In CNA6 rice, the raw trial-mean yield trend
was positive, and the common-check sensitivity analysis provided the
more precise programme comparison. The within-trial
candidate-minus-check sensitivity analysis estimated about 235 kg/ha per
cycle and excluded zero. Flowering and height trends remained
environment-dependent, as expected in a design where cycle and calendar
year coincide.
...
aggregate rather than an independent reanalysis.
...
presenting this as a causal selected-versus-unselected comparison would
be invalid.

The Genomes-to-Fields analysis froze an index on 2014–2017
environment-adjusted means and evaluated the same genotypes in
2018–2021. Only 53 genotypes met the predeclared requirement of at least
two environments in both periods. The early-to-late score correlation
was about 0.56. Future yield response was favourable, while anthesis and
anthesis-silking interval responses showed the expected environment
sensitivity. This provides independent candidate-level transport
evidence that complements the recurrent-cycle datasets.
...
with 0.158 in the paper, and 0.253 t/ha per cycle from C1 to C4,
compared with 0.225. C0–C3 adjusted means were within 0.01 t/ha of the
published table. The C4 deposit contains 44 candidate entries whereas
the paper reports 43; retaining 43 rather than all 44 changed the
...
complete environment. Direction search occurred only within the other
...
- Trials or genotypes are the independent units; plot rows are not
  treated as independent selection cycles.
- Every temporal index is frozen before the validation period.
- Adjusted-mean covariance is called a working surrogate, not additive
  genetic covariance.
- Common-check, yield-only and out-of-time comparisons were specified in
...
| endpoint | status | interpretation | prospective study design |
|:---|:---|:---|:---|
| incremental field response among competing desired-gain strategies | future-evidence-opportunity | Existing evidence validates the mathematical, genomic, simulation and out-of-sample recommendation layers. A prospective comparison would add a direct estimate of incremental field response among strategies. | Study design: assign comparable populations to competing vectors before selection and evaluate their progeny in common trials. |

The current evidence supports DesiredGainR as rigorous, auditable
decision support: its mathematics, optimisation, genomic signal,
cycle-gain reproduction and out-of-sample recommendation behaviour have
all been tested. The probabilities are interpreted correctly as support
under the fitted genetic model and declared breeding scenario.
...
incremental field response among strategies and provide an additional
layer of programme-specific calibration. It does not negate the
mathematical, simulation and real-programme evidence already established
here.
