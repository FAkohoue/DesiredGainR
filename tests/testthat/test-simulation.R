...
make_phased <- function(n_ind = 24L, n_var_per_chr = 30L, n_chr = 2L,
  ...
  draw <- function() {
    matrix(
      stats::rbinom(n_var * n_ind, 1L, 0.5), nrow = n_var,
...
test_that("heterozygous calls are never resolved silently", {
  ...
  masked <- haplotypes_from_inbred_dosage(
    dosage, heterozygous_policy = "mask"
  )
...
test_that("no arbitrary heterozygosity threshold is imposed", {
  set.seed(4)
  dosage <- matrix(
    sample(c(0L, 1L, 2L), 200L, replace = TRUE), nrow = 20L,
...
test_that("masked dosage flows into founder_haplotypes under a policy", {
  ...
  n_var <- 12L
  dosage <- matrix(
    sample(c(0L, 2L), n_var * 4L, replace = TRUE), nrow = n_var,
...
test_that("an uninformative map column name gives an actionable error", {
  ...
  founders <- founder_haplotypes(
    phased$hap1, phased$hap2, renamed, variant_col = "SNP"
  )
...
test_that("founder_population calibrates trait variance to G", {
  ...
  setup <- founder_population(
    founders, G = G, h2 = c(YLD = 0.3, DIS = 0.5),
...
test_that("founder_population restores the caller's RNG state", {
  ...
  invisible(founder_population(
    founders, G = G, h2 = 0.4, n_qtl_per_chromosome = 10L, seed = 5L
  ))
  expect_identical(
...
test_that("simulation records gain and diversity across cycles", {
  ...
  traits <- c("YLD", "DIS")
  G <- matrix(
    c(1.00, -0.25, -0.25, 0.64), 2, dimnames = list(traits, traits)
  )
  setup <- founder_population(
    founders, G = G, h2 = c(YLD = 0.4, DIS = 0.4),
  ...
  expect_equal(nrow(simulation$cycle_table), 3L * 2L)
  expect_equal(sort(unique(simulation$cycle_table$cycle)), 1:3)
  ...
  # Gain must accumulate: cycle three must be further from the founders than
  # cycle one. This is the robust directional claim at this population size.
  yield_by_cycle <- simulation$cycle_table[
    trait == "YLD", cumulative_gain
  ]
  expect_gt(yield_by_cycle[3L], yield_by_cycle[1L])
  ...
  expect_length(variance_by_cycle, 3L)
...
test_that("the clonal system requires a dominance setup", {
  ...
  additive_only <- founder_population(
    founders, G = G, h2 = 0.4, n_qtl_per_chromosome = 10L, seed = 2L
  )
  expect_error(
    simulate_selection_cycles(
      additive_only, c(A = 1, B = 1), n_cycles = 1L,
...
test_that("desired-gain direction changes the realised gain profile", {
  ...
  traits <- c("YLD", "DIS")
  G <- matrix(
    c(1.00, -0.40, -0.40, 0.64), 2, dimnames = list(traits, traits)
  )
  setup <- founder_population(
    founders, G = G, h2 = c(YLD = 0.4, DIS = 0.4),
    n_qtl_per_chromosome = 20L, seed = 3L
  ...
  run <- function(d, seeds = c(11L, 12L, 13L, 14L)) {
    gains <- vapply(seeds, function(s) {
      simulate_selection_cycles(
        setup, d, n_cycles = 3L, mating_system = "outcross",
        n_parents = 10L, n_crosses = 20L, n_progeny_per_cross = 5L,
