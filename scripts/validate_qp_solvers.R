...
decode_bound <- function(x) {
  vapply(
    x,
    function(value) {
      if (identical(value, "Inf")) {
        Inf
      } else if (identical(value, "-Inf")) {
        -Inf
      } else {
        as.numeric(value)
      }
...
make_problem <- function(
  ...
  list(
    ...
    H = unname(H),
    f = as.numeric(f),
    lower = encode_bound(lower),
    upper = encode_bound(upper),
    class = class,
    seed = seed,
    condition_number = kappa(H, exact = TRUE),
    expected = expected
  )
}
...
add_problem(
  "analytic_narrow_interval",
  diag(2),
  c(-1, 1),
  c(0.2, -0.300000000001),
  c(0.200000000001, -0.3),
  "analytical",
  4L,
  c(0.200000000001, -0.300000000001)
)
...
for (seed in 101:120) {
  ...
  p <- sample(2:6, 1L)
  M <- matrix(stats::rnorm(p^2), nrow = p)
  ...
  gradient[state == "fixed"] <- stats::runif(
    sum(state == "fixed"),
    -2,
    2
  )
  ...
  gradient[state == "lower"] <- stats::runif(
    sum(state == "lower"),
    0.1,
    2
  )
  ...
  gradient[state == "upper"] <- -stats::runif(
    sum(state == "upper"),
    0.1,
    2
  )
...
for (condition_target in condition_targets) {
  for (replicate in 1:3) {
    seed <- as.integer(
      1000 + log10(condition_target + 1) * 10 + replicate
    )
    set.seed(seed)
    
    p <- 5L
    Q <- qr.Q(qr(matrix(stats::rnorm(p^2), nrow = p)))
    ...
    problem_class <- if (condition_target <= 1e4) {
      "well_conditioned"
    } else {
      "conditioned"
    }
...
for (distance in c(0, 1e-12, 1e-10, 1e-8)) {
  ...
  add_problem(
    paste0(
      "near_bound_",
      format(distance, scientific = TRUE)
    ),
    H,
    f,
    lower,
    upper,
    "degenerate_active_set",
    2001L,
    x
  )
}
...
if (length(base_index) != 1L) {
  stop(
    "Could not identify the active_101 invariance reference problem.",
    call. = FALSE
  )
}

base <- problems[[base_index]]
H <- as.matrix(base$H)
f <- unlist(base$f)
lower <- decode_bound(base$lower)
upper <- decode_bound(base$upper)
p <- length(f)
...
# Preserve the exact executable path supplied by the workflow. Do not use
# normalizePath(), because it may dereference an action-managed launcher.
...
# R can modify the dynamic-library search path on Unix-like systems. The
# independent Python process must not inherit R's LD_LIBRARY_PATH or
# LD_PRELOAD, because these can mask libraries used by Python extension
# modules such as scipy and ctypes.
python_env <- c(
  "PYTHONNOUSERSITE=1",
  "PYTHONHOME=",
  "PYTHONUSERBASE="
...
if (
  is.null(oracle_payload$versions) ||
  is.null(oracle_payload$results)
) {
  stop(
    "The Python solver oracle returned an incomplete result payload.",
    call. = FALSE
  )
}
...
if (length(missing_results)) {
  stop(
    "The Python solver oracle did not return results for: ",
    paste(missing_results, collapse = ", "),
    call. = FALSE
  )
}

if (length(unexpected_results)) {
  stop(
    "The Python solver oracle returned unexpected results for: ",
...
rows <- lapply(
  ...
  function(problem) {
    ...
    internal <- DesiredGainR:::.dgr_box_qp(
      H,
      f,
      lower,
      upper
    ...
    summarise <- function(result, solver) {
      if (
        ...
        is.null(result$dual) ||
        is.null(result$objective) ||
        is.null(result$status)
      ) {
        stop(
          "Incomplete oracle result for problem ",
          problem$problem_id,
          " and solver ",
          solver,
          ".",
      ...
      lower_dual <- numeric(length(x))
      upper_dual <- numeric(length(x))
      
      if (grepl("clarabel", solver, fixed = TRUE)) {
        if (identical(solver, "clarabel_reordered")) {
          dual <- rev(dual)
        }
    ...
    clarabel <- summarise(
      oracle_result$clarabel,
      "clarabel"
    )
    
    clarabel_reordered <- summarise(
      oracle_result$clarabel_reordered,
      "clarabel_reordered"
    )
    
    osqp_unpolished <- summarise(
      oracle_result$osqp_unpolished,
      "osqp_unpolished"
    )
    
    osqp_polished <- summarise(
      oracle_result$osqp_polished,
      "osqp_polished"
    )
    ...
    if (problem$class == "degenerate_active_set") {
      x_tolerance <- 1e-5
    }
    
    objective_tolerance <- if (condition_number <= 1e8) {
      1e-9
    } else {
      1e-6
    }
    ...
    x_scale <- max(
      1,
      abs(internal$solution),
      abs(clarabel$x)
    )
    
    objective_scale <- max(
      1,
      abs(internal$objective),
      abs(clarabel$objective)
    )
    
    solution_difference <- max(
      abs(internal$solution - clarabel$x)
    )
    
    osqp_unpolished_solution_difference <- max(
      abs(internal$solution - osqp_unpolished$x)
    )
    
    osqp_polished_solution_difference <- max(
      abs(internal$solution - osqp_polished$x)
    )
    
    objective_difference <- abs(
      internal$objective - clarabel$objective
    )
    ...
    osqp_kkt_tolerance <- if (condition_number >= 1e6) {
      2e-6
    } else {
      1e-6
    }
    ...
    fixed <- is.finite(lower) &
      is.finite(upper) &
      lower == upper
    
    internal_gradient <- as.numeric(
      H %*% internal$solution + f
    )
    
    internal_lower_dual <- ifelse(
      is.finite(lower) &
        abs(internal$solution - lower) <= active_tolerance,
      pmax(internal_gradient, 0),
      0
    )
    ...
    dual_difference <- function(result) {
      ...
      max(
        abs(
          c(
            internal_lower_dual[!fixed] -
              result$lower_dual[!fixed],
            internal_upper_dual[!fixed] -
              result$upper_dual[!fixed]
          )
        )
      )
    }
    
    clarabel_reordered_solution_difference <- max(
      abs(clarabel$x - clarabel_reordered$x)
    )
    ...
    pass <- grepl(
      ...
      grepl(
        "Solved",
        clarabel_reordered$status,
        ignore.case = TRUE
      ) &&
      grepl(
        "solved",
        osqp_unpolished$status,
        ignore.case = TRUE
      ) &&
      grepl(
        "solved",
        osqp_polished$status,
        ignore.case = TRUE
      ) &&
      internal$kkt$overall <= 1e-7 &&
      clarabel$kkt <= clarabel_kkt_tolerance &&
      ...
      (
        !require_solution ||
          solution_difference <= x_tolerance * x_scale
      ) &&
      (
        !require_solution ||
          osqp_polished_solution_difference <= x_tolerance * x_scale
      ) &&
      clarabel_reordered_solution_difference <=
      x_tolerance * x_scale &&
      (
        is.na(analytical_difference) ||
          analytical_difference <= x_tolerance
      )
    ...
    data.frame(
      ...
      clarabel_version = versions$clarabel,
      ...
      relative_solution_difference = solution_difference / x_scale,
      clarabel_reordered_solution_difference =
        clarabel_reordered_solution_difference,
      osqp_unpolished_solution_difference =
        osqp_unpolished_solution_difference,
      osqp_polished_solution_difference =
        osqp_polished_solution_difference,
      relative_objective_difference =
        objective_difference / objective_scale,
      ...
      osqp_unpolished_dual_difference =
        dual_difference(osqp_unpolished),
      osqp_polished_dual_difference =
        dual_difference(osqp_polished),
...
if (!all(report$pass)) {
  ...
  stop(
    sum(!report$pass),
    " independent solver validation problem(s) failed.",
    call. = FALSE
  )
}
